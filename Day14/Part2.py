...
for line in input:
    ...
    for pair_str in pair_strs:
        pair = pair_str.split(',')
        max_depth = max(max_depth, int(pair[1]))

map = [[0] * 1000 for _ in range(max_depth + 1)]
map.append([ROCK] * 1000)
total = 0
...
for line in input:
    ...
    for pair_str in pair_strs:
        pair = pair_str.split(',')
        pairs.append((int(pair[0]), int(pair[1]) - 1))
    # print(pairs)
...
while True:
    ...
    while new_sand_y + 1 < 1000 and 0 <= new_sand_x < 1000 and (map[new_sand_y + 1][new_sand_x] == 0 or map[new_sand_y + 1][new_sand_x - 1] == 0 or map[new_sand_y + 1][new_sand_x + 1] == 0):
        if map[new_sand_y + 1][new_sand_x] == 0:
            ...
            new_sand_y += 1
            new_sand_x += 1
    if (new_sand_x == 500 and new_sand_y == -1):
        total += 1
