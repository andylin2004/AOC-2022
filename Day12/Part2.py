...
for line in range(len(input)):
    ...
    for char in range(len(input[line])):
        ...
        if input[line][char] == "E":
            end_x = line
            end_y = char
        elif input[line][char] == "a":
            a_locations.append((line, char))
    array.append(line_arr)


def dij(array, x, y): #dijkastra algo for pathcounting
    ...
    pq.append([0, (x, y)]) # preprocessing start place so we can kickstart checking the neighbors later
    results[x][y] = 0 # preprocessing the start, as it doesn't count at all and we don't want anything bad to happen
...
min_num = None
for i in a_locations:
    if min_num is None:
        min_num = dij(array, i[0], i[1])
    else:
        min_num = min(min_num, dij(array, i[0], i[1]))
